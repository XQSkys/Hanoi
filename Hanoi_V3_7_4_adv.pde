...
//rechter turm
  ...
  int check=0;
  int seeC=0;    //variable für die erkennung der steine, nicht standartkonforme verwendung der get(), ist bei dem farbwert weiss anders als bei allen anderen farben
  int x=0;
...
fill(0, 200, 0);                       //malt den Load button grün über
       ...
       if (mousePressed) {                   //wenn die maus auf den kooardinaten des Load buttons, er ist nun grün, und ein mausklick gemacht wird tue:
        ...
        PImage bild;                         //Aktivire den bildspeicher mit dem namen bild
       
           bild = loadImage(runden+"bild.png");          //lade in den bild speicher das bild mit dem namen bild.png aus dem haubptordner
    ...
    image(bild, 0, 0);  // Füge das bild ein -- vollständiges ersetzen des programm fenster inhalts
        ...
        else{//wenn kein bild gefunnden wurde also bild==0
          fill(200, 0, 0);  //wwähle farbe rot
         rect(570, 50, 70, 40 ); //übermale das load fenster rot
      ...
      save(runden+"bild.png");  //speichert den inhalt des hanoi fensters im haubtferzeichnis des spiels unter den namen bild.png
...
// hier beginnt die erzeungung und funktionalisirung des "Höhe" button:
   ...
   fill(0, 200, 0);  //setze farbe auf grün
        ...
        runden++;   //zähle zur ruhenzahl 1 dazu
     if (runden>=20) runden=1; // wenn die zahl größer oder gleich 20 ist setze runden wieder auf 1 runter
...
//wichtig: hier beginnt das malen der steine
    ...
    if (xxx==0) { //hier beginnt das malen der spielsteine
      ...
      for (int y=130; check<1; y++) {//forschleife um von oben nach unten die auf den steinauflage kooardinate zu finden
        ...
        if (seeC<=-2) {//eigenich eine fehlerhafte funktionswe aber weiss hat einen anderen negativen wert als alle anderen farben wenn man int benutzt stat color
...
if(rand==1)// hier ist die funktion die die rechtecke zeichnet für den Chaos modus
{
  ...
  rect((50 + breite/2) + randpos,diay2-choch , size, 60-runden*2);//das ist eine schwirigefunktion die die steine in abhängiketin von position(0-2 rand und den anderen höhe und breite variablen macht.)
      ...
      xxx++;//erhöht den zähler um die variable damit die for schleife endet die die türme zeichnet
         saveFrame(runden+"save/"+runden+"bild####.png");
 ...
 fill(0, 200, 0);                       //malt den Load button grün über
  ...
  saveFrame(runden+"SavesMovie/Movie######.png");
    ...
    fill(0,200, 0);
          ...
          if (mousePressed) {
           ...
           rect(440, 600, 170, 50);
            click=1;
            activ3=0;
            delay(66);  saveFrame(runden+"save/"+runden+"bild####.png");
          }
        }
